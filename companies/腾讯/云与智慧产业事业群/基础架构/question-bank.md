...
### 项目深挖方向（社招核心）
1. 你做过的最复杂的性能优化是什么？线程池/连接池怎么调优的？
2. 批处理场景下如何分配线程？5万条数据的分页拉取+多线程并行处理策略？
3. 自研框架/中间件的设计决策？和开源方案（如MQ）的取舍？
4. 分库分表（滚表）的设计？查询键选择？分片路由规则？
5. 异步任务的状态一致性问题？worker拉取后任务被取消的竞态如何处理？

### 工程实践（社招二面高频）
6. 项目重构的策略？如何评估重构范围？渐进式重构怎么做？
7. 数据库Schema变更后灰度发布，如何保证新旧版本代码兼容？（冗余字段→双写→清洗→切换）
8. RESTful API设计原则？为什么需要自定义业务错误码而不只用HTTP状态码？
9. 线上问题排查的完整流程？日志→监控→链路追踪→火焰图？
10. 慢SQL排查：slow_query_log → EXPLAIN → type/key/rows/Extra → 索引优化/SQL改写？
11. QPS优化的常见手段？批量拉取+并行处理+连接池复用+减少锁竞争？
12. 连接池问题如何判断？活跃连接数、等待队列长度、连接获取超时率的监控？

### 分布式故障定位（社招开放题）
13. 多个客户端连接服务端，部分成功部分失败，如何判断是服务端还是客户端问题？
14. 客户端连上后掉线，服务端的处理策略？心跳检测+重连机制+会话保持？
15. 分布式系统中的灰度发布策略？金丝雀发布 vs 蓝绿部署 vs 滚动更新？

> 来源：CSDN腾讯CSIG社招面经（Java转Go，1年经验）、CSDN腾讯CSIG-腾讯云后台开发面经（非科班，Go方向，已offer）
...
3. 设计一个高可用的API网关
4. 设计一个分布式日志收集系统
5. 设计一个异步任务调度框架（支持任务状态查询、重试、取消）
6. 设计一个多租户资源隔离方案（CSIG云平台场景）
...
## 备注
- 来源：CSDN腾讯CSIG-腾讯云-后台开发面经（已offer）、知乎csig腾讯云暑期实习面经、CSDN腾讯CSIG暑期后端实习面经（已offer）、牛客腾讯云社招Golang面经、腾讯云开发者社区K8s面试题汇总、CSDN腾讯CSIG社招面经（Java转Go）
- CSIG后端面试特点：Go语言是核心（即使简历写Java也会问Go）、云原生技术栈是区分度最高的考点、项目深挖占比大
- 社招与校招的核心区别：社招不考八股文，全程围绕"你做了什么、为什么这么做、还能怎么做"展开
